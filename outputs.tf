output "mfa_policy_arn" {
  ...
  value       = element(concat(aws_iam_policy.mfa[0].arn, [""]), 0)
}

output "trail_arn" {
  description = "Cloud trail arn."
  value       = element(concat(aws_cloudtrail.cloudtrail[0].arn, [""]), 0)
}

output "cloudwatch_log_group_arn" {
  description = "Cloud trail arn."
  value       = element(concat(aws_cloudwatch_log_group.log_group[0].arn, [""]), 0)
}

output "monitor_readonly_user_arn" {
  description = "ARN for the monitor readonly user"
  value       = element(concat(aws_iam_user.monitor_readonly_user[0].arn, [""]), 0)
}
...
output "monitor_readonly_user_access_key_id" {
  ...
  value = element(
    concat(
      aws_iam_access_key.monitor_readonly_user_access_key[0].id,
      [""],
    ),
    0,
  )
...
output "monitor_readonly_user_secret_access_key" {
  ...
  value = element(
    concat(
      aws_iam_access_key.monitor_readonly_user_access_key[0].secret,
      [""],
    ),
    0,
  )
